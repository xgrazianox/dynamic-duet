-- =========================================================================
-- F6-r2 A2 — update_portfolio_settings: UNICA via di modifica dei settings.
-- Ordine: proprietà → lock portfolio → idempotenza → validazione → update.
-- L'UPDATE diretto client su portfolio_settings viene eliminato (policy+grant).
-- =========================================================================
...
BEGIN
  ...
  -- Chiavi: rifiuto esplicito di campi di sistema o sconosciuti
  ...
  -- LOCK prima dell'idempotenza
  PERFORM 1 FROM public.portfolios WHERE id = v_pid FOR UPDATE;

  -- Hash server-side su payload canonico (chiavi ordinate)
  ...
  IF _payload ? 'default_fx' THEN
    ...
    END LOOP;
  END IF;
  -- driver: appartenenza, attivi, distinti (valuto lo stato FINALE combinato)
  ...
  IF (_payload ? 'msci_instrument_id') OR (_payload ? 'gold_instrument_id') THEN
    ...
    END IF;
  END IF;
  -- engine_config: completa e valida secondo il modello consumato da signalEngine
  IF _payload ? 'engine_config' THEN
    ...
    -- interi positivi (finestre/conferme/lookback/voti)
    ...
    FOREACH v_k IN ARRAY ARRAY['b1SmaMonths','b2SmaMonths','b3VolLookback','minVotesRequired','confirmMonths'] LOOP
      ...
      END IF;
    END LOOP;
    -- bande/soglie: numerici, banda >= 0, soglia > 0
  ...
  -- ===== no-op dichiarato: contenuto già uguale allo stato persistito =====
...
-- Eliminazione dell'UPDATE diretto client (lettura owner conservata)
