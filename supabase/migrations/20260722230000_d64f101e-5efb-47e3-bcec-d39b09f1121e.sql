-- =========================================================================
-- FASE 3 — BLOCCO F3-0 (backend)
-- 1) RPC save_target_set        (authenticated + service_role)
-- 2) RPC persist_regime_decision (service_role ONLY)
-- 3) Trigger protezione price_points (righe 'opening' / source_batch_id)
-- 4) Trigger protezione campi identità instruments + no-delete client
-- Convenzioni identiche alle RPC F1: SECURITY DEFINER, search_path='',
-- oggetti qualificati public.*, lock FOR UPDATE, idempotenza mutation_requests.
-- =========================================================================

-- =========================================================================
-- 1) RPC save_target_set(_key text, _payload jsonb)
--    _payload = { "regime": "RISK_ON"|"RISK_OFF",
--                 "rows": [ {"instrument_id": <uuid>|null, "weight": <number>}, ... ] }
--    - portfolio derivato da auth.uid() (client portfolio_id ignorato)
--    - esattamente una riga Cash (instrument_id NULL)
--    - nessuno strumento duplicato; solo strumenti 'active' del portafoglio
--    - compatibilita' regime: RISK_ON→{AGGRESSIVE,BOTH}, RISK_OFF→{DEFENSIVE,BOTH}
--    - somma pesi NUMERIC esatta = 100.0000
--    - version = max(version)+1 su (portfolio,regime); effective_from = CURRENT_DATE
--    - supersede della precedente 'active' E della bozza 'draft' seed
--    - hash idempotenza su righe ordinate canonicamente + regime
-- =========================================================================
...
BEGIN
  ...
  -- Ordine canonico per hash idempotente (indipendente dall'ordine del client)
  ...
  -- Ogni strumento: appartiene al portafoglio, active, compatibile col regime
  ...
  -- Supersede della precedente active E della bozza draft (stesso regime),
  -- PRIMA dell'insert per rispettare l'indice unico "una active per regime".
...
-- =========================================================================
-- 2) RPC persist_regime_decision(_payload jsonb) — SERVICE_ROLE ONLY
--    Chiamata solo dalla Edge Function evaluate_regime (identita' dal JWT).
--    _payload = { actor_user_id, portfolio_id, as_of_month,
--                 regime_a, regime_b, final_regime,  -- final_regime NULL = UNDETERMINED
--                 decision_mode, config, config_hash, input_fingerprint, engine_version }
--    - riverifica portfolio↔actor
--    - lock, is_switch vs ULTIMA decisione DETERMINATA (final_regime NOT NULL)
--    - idempotente su (portfolio, as_of_month, config_hash, input_fingerprint)
-- =========================================================================
...
DECLARE
  ...
  v_pid uuid;
  v_month date;
  v_final public.regime;
  v_prev_final public.regime;
  v_is_switch boolean;
  v_id uuid;
  v_existing_id uuid;
BEGIN
  ...
  -- Doppia verifica attore↔portafoglio
  ...
  -- Serialize per calcolo is_switch coerente
  PERFORM 1 FROM public.portfolios WHERE id = v_pid FOR UPDATE;

  -- Idempotenza sull'identita' logica
  SELECT id INTO v_existing_id FROM public.regime_decisions
   WHERE portfolio_id = v_pid
     AND as_of_month = v_month
  ...
  IF v_final IS NULL THEN
    -- UNDETERMINED: persistita, non e' mai uno switch
    v_is_switch := false;
  ELSE
    -- ultima decisione DETERMINATA anteriore (le UNDETERMINED intermedie non rompono il confronto)
    SELECT final_regime INTO v_prev_final
    FROM public.regime_decisions
    WHERE portfolio_id = v_pid
      AND final_regime IS NOT NULL
      AND as_of_month < v_month
  ...
  IF v_id IS NULL THEN
    -- corsa: inserita concorrentemente → ritorna quella esistente
...
-- =========================================================================
-- 3) Protezione price_points: righe 'opening' e batch immutabili lato client
--    Bypass per le RPC autoritative (current_user = 'postgres', owner delle RPC).
-- =========================================================================
...
BEGIN
  IF current_user = 'postgres' THEN
    RETURN COALESCE(NEW, OLD);  -- RPC autoritative (import/amend opening)
  END IF;
  ...
  -- UPDATE of an already-protected row (including upsert ON CONFLICT DO UPDATE
  -- that lands on an opening/batch key): immutable for clients — checked FIRST
  ...
  -- INSERT / UPDATE: clients may not create/set the protected shape
...
-- =========================================================================
-- 4) Protezione instruments: identita' immutabile una volta referenziato,
--    archiviazione vietata se peso positivo in un target attivo,
--    nessun DELETE fisico lato client.
-- =========================================================================
...
BEGIN
  ...
  -- Campi identita' immutabili se referenziato da operazioni o prezzi
...
AS $$
BEGIN
  -- I client non hanno privilegio DELETE; questo trigger e' difesa in profondita'.
  -- I cascade legittimi (eliminazione portafoglio/utente) girano come 'postgres'.
  IF current_user = 'postgres' THEN
    RETURN OLD;
  END IF;
...
-- =========================================================================
-- 5) Protezione fx_rates: righe batch (opening) immutabili lato client.
--    fx_rates non ha colonna 'source': il marcatore opening è source_batch_id.
--    Simmetrico a price_points; bypass per le RPC autoritative (owner postgres).
-- =========================================================================
...
BEGIN
  IF current_user = 'postgres' THEN
    RETURN COALESCE(NEW, OLD);  -- import/amend opening (batch propri)
  END IF;
  ...
  -- UPDATE di una riga batch esistente (incl. upsert che colpisce una chiave
  -- opening, e il tentativo di "trasformarla" in cambio manuale): immutabile.
  ...
  -- INSERT / UPDATE: il client non può assegnare source_batch_id
  IF NEW.source_batch_id IS NOT NULL THEN
