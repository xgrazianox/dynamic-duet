-- =========================================================================
-- HOTFIX F5 — ordine transazionale delle RPC di regime:
--   proprietà → LOCK portfolio → idempotenza → mutazione
-- (prima: proprietà → idempotenza → lock). Il retry di una chiave in volo ora
-- attende il commit dell'originale sul lock di portafoglio e riceve il cached.
-- INVARIATI: firme, payload, semantica, SECURITY DEFINER, search_path='',
-- qualificazione public.*, grant/revoke, mutation_requests, risultati.
-- =========================================================================
...
BEGIN
  ...
  -- LOCK PRIMA dell'idempotenza (hotfix F5)
